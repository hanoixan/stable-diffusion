...
def capture_img(video_device, h0, w0):
    ret, frame_raw = video_device.read()

    # crop center to square
    shape = frame_raw.shape
    dim = min(shape[1], shape[0])
    x = shape[1]/2 - dim/2
    y = shape[0]/2 - dim/2
    frame_raw = frame_raw[int(y):int(y+dim), int(x):int(x+dim)]
...
# Logging
logger(vars(opt), log_csv = "logs/cam2img_logs.csv")
...
while True:
    # capture from device
    init_image = capture_img(vid, opt.H, opt.W).to(opt.device)
            ...
            for prompts in tqdm(data, desc="data"):
                ...
                with precision_scope("cuda"):
                    ...
                    for i in range(batch_size):
                        ...
                        out_frame_array = x_sample.astype(np.uint8)
                        out_frame = Image.fromarray(out_frame_array)

                        open_cv_image = np.array(out_frame) 
                        # Convert RGB to BGR 
